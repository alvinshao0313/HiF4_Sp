 ...
 * mx\_ops.py: functions to convert Pytorch Tensors to an MX-compatible data format.
   The direct APIs include ```quantize_mx_op```, ```get_mx_quantize_params```,
   and ```apply_mx_quantize_with_param```.
 ...
 * ```A_elem_format```: MX element format for the first input to ```matmul```.
 * ```B_elem_format```: MX element format for the second input to ```matmul```.
 ...
 * ```a_scale_mode```: Shared scale format for activation quantization in
  ```linear```.
 * ```w_scale_mode```: Shared scale format for weight quantization in
  ```linear```.
 * ```A_scale_mode```: Shared scale format for the first input to ```matmul```.
 * ```B_scale_mode```: Shared scale format for the second input to ```matmul```.
 * ```block_size```: MX scaling block size. Default is 0.
  A value of 0 means the entire dim shares a single exponent.
  For MX-compatible formats this should be 32. In ```quantize_mx_op```,
  ```block_size=-1``` selects the custom CUDA per-channel path.
 * ```shared_exp_method```: Method to compute the MX shared exponent.
  For MX-compatible formats this should be "max". Default "max".
 * ```mx_flush_fp32_subnorms```: Flush MX blocks with subnormal FP32 shared
  scales to zero. Default False.
 * ```per_tensor```: Use one shared quantization range for the whole tensor in
  the custom CUDA path. Default False.
 * ```double_quant```: Quantize the MX scale values separately before applying
  the MX element quantization. Default False.

The supported element format strings include:
 * ```fp8_e4m3```
 * ```fp8_e5m2```
 * ```fp6_e3m2```
 * ```fp6_e2m3```
...
The supported shared scale modes are:
 * ```0```: PoT shared scale. This is the default MX mode.
 * ```3```: PoT-R shared scale.
 * ```2```: FP16/full-precision shared scale computed from the tile maximum.
 * ```152```: FP8 E5M2 shared scale.
 * ```143```: FP8 E4M3 shared scale.

Asymmetric element formats select a different internal quantization rule based
on ```scale_mode```. With ```scale_mode``` 0 or 3, asymmetric floating-point
formats use separate positive and negative PoT scales, and asymmetric integer
formats use an asymmetric PoT integer scale. With ```scale_mode``` 2, 143, or
152, asymmetric floating-point formats use separate positive and negative
floating-point scales, and asymmetric integer formats use an asymmetric
integer scale. The synced-mantissa scale modes 11, 111, 1111, 11111, 111111,
and 1111111 are only implemented for asymmetric formats.
...
Non-default ```scale_mode``` values and asymmetric formats are primarily
implemented in the custom CUDA path. The pure PyTorch path currently supports
the basic PoT MX behavior only.
The direct CUDA parameter APIs do not implement ```per_tensor```. Per-channel
quantization is selected by ```block_size=-1``` in ```quantize_mx_op```.
...
```
```
# MXFP4_e2m1 with FP8 E4M3 shared scales
mx_specs = MxSpecs()

mx_specs["scale_bits"] = 8
mx_specs["w_elem_format"] = "fp4_e2m1"
mx_specs["a_elem_format"] = "fp4_e2m1"
mx_specs["w_scale_mode"] = 143
mx_specs["a_scale_mode"] = 143
mx_specs["block_size"] = 32
mx_specs["custom_cuda"] = True
```
```
# Direct tensor quantization with quantize_mx_op
import torch
from mx.specs import MxSpecs
from mx.mx_ops import quantize_mx_op

x = torch.randn(4, 128, device="cuda", dtype=torch.float32)
mx_specs = MxSpecs(scale_bits=8, block_size=32, custom_cuda=True)
qx = quantize_mx_op(
    x,
    mx_specs,
...
```
```
# Reuse CUDA quantization parameters
from mx.mx_ops import get_mx_quantize_params, apply_mx_quantize_with_param

y = torch.randn_like(x)
scale1, scale2, shift, qx = get_mx_quantize_params(
    x,
    mx_specs,
    elem_format="fp4_e2m1_asym",
    axes=[-1],
    scale_mode=152,
    round=mx_specs["round_mx_output"],
)
qy = apply_mx_quantize_with_param(
    y,
    scale1,
    scale2,
    shift,
...
```get_mx_quantize_params``` returns the scale and shift tensors used by the
custom CUDA quantizer plus the quantized tensor. ```apply_mx_quantize_with_param```
applies those parameters to another tensor with the same tiling layout. These
APIs currently require CUDA tensors and ```custom_cuda=True```.
...
### Double Quantization ###
When ```double_quant``` is True, the library first gets the MX quantization
parameters for the original tensor and then quantizes the scale tensor itself.
This mode is implemented in ```linear``` and ```matmul```. It does not support
asymmetric element formats. The scale format for double quantization is limited
to ```scale_mode``` 0, 143, or 152.
