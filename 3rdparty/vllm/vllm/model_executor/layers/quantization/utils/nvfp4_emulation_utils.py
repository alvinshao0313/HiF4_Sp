...
# Compat alias for pre-v0.27 call sites that imported the bare tensor name.
kE2M1ToFloat = kE2M1ToFloat_handle.val


def _nvfp4_triton_fp8e4nv_supported() -> bool:
    """Triton ``tl.float8e4nv`` requires SM89+; A800 (SM80) must use the Python path.

    Numerical rules are unchanged: this only selects the existing upstream
    Python reference implementation already used when CUDA-alike is false.
    """
...
def dequantize_to_dtype(
    ...
    if not swizzle and _nvfp4_triton_fp8e4nv_supported():
