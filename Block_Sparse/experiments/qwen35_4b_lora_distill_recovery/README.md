...
> 正式技术报告：[`reports/paper_report_lora_distill_recovery.md`](reports/paper_report_lora_distill_recovery.md)
> 机器可读指标：[`reports/metrics_tables.json`](reports/metrics_tables.json)
> 本 README 为 lineage + 配置摘要 + 指针，详细分析与结论见正式报告。
...
└── reports/
    ├── paper_report_lora_distill_recovery.md  # 正式技术报告
    ├── lora_distill_recovery_report.md         # 早期草稿（保留历史）
    └── metrics_tables.json                    # 机器可读指标
```
