...
class LayerParameters(nn.Module):
    def __init__(self, base, sharing: str, *, lora_mode: str = "both",
                 lora_rank: int = 4, lora_alpha: float = 8.0):
        super().__init__()
        if sharing != "group":
            raise ValueError(sharing)
        if lora_mode not in {"attention", "moe", "both", "none"}:
            raise ValueError(lora_mode)
        if lora_rank <= 0 or lora_rank >= base.spec.hidden_size:
            raise ValueError("invalid LoRA rank")
        if not torch.isfinite(torch.tensor(float(lora_alpha))) or lora_alpha <= 0:
        ...
        hidden = int(base.spec.hidden_size)
        self.attention_lora_A = nn.Parameter(torch.empty(lora_rank, hidden))
        self.attention_lora_B = nn.Parameter(torch.zeros(hidden, lora_rank))
        self.moe_lora_A = nn.Parameter(torch.empty(lora_rank, hidden))
        self.moe_lora_B = nn.Parameter(torch.zeros(hidden, lora_rank))
        nn.init.kaiming_uniform_(self.attention_lora_A, a=5 ** 0.5)
        nn.init.kaiming_uniform_(self.moe_lora_A, a=5 ** 0.5)
        if lora_mode not in {"attention", "both"}:
            self.attention_lora_A.requires_grad_(False)
            self.attention_lora_B.requires_grad_(False)
        if lora_mode not in {"moe", "both"}:
            self.moe_lora_A.requires_grad_(False)
            self.moe_lora_B.requires_grad_(False)

    def snapshot(self) -> dict[str, torch.Tensor]:
        return {name: p.detach().cpu().clone() for name, p in self.state_dict().items()}

    def lora(self, x: torch.Tensor, branch: str) -> torch.Tensor:
        if branch == "attention":
            a, b = self.attention_lora_A, self.attention_lora_B
            enabled = self.lora_mode in {"attention", "both"}
        elif branch == "moe":
            a, b = self.moe_lora_A, self.moe_lora_B
            enabled = self.lora_mode in {"moe", "both"}
        else:
            raise ValueError(branch)
        if not enabled:
            return torch.zeros_like(x)
        z = torch.nn.functional.linear(x.float(), a.float())
        return (torch.nn.functional.linear(z, b.float()) * (self.lora_alpha / self.lora_rank)).to(x.dtype)
