...
class Output:
    ...
    attention_lora: torch.Tensor
    moe_lora: torch.Tensor


class Student(nn.Module):
    def __init__(self, base, sharing="group", *, rms_norm_eps=1e-6,
                 lora_mode="both", lora_rank=4, lora_alpha=8.0):
        super().__init__()
        self.base = base
        self.learned = LayerParameters(base, sharing, lora_mode=lora_mode,
    ...
    def forward(self, hidden, *, position_embeddings, attention_mask=None, use_ste=True):
        ...
        attention_lora = self.learned.lora(hidden, "attention")
        # Match vLLM's deferred residual representation: correct the bypass
        # after computing its norm, before adding the main branch.
        residual = (hidden + attention_lora) + self.attention(
            normed, position_embeddings, attention_mask, use_ste=use_ste)
        normed = _rms_norm(residual, self.learned.moe_norm, self.eps)
        result, logits, counts = self.moe(normed, use_ste=use_ste)
        moe_lora = self.learned.lora(residual, "moe")
        return Output((residual + moe_lora) + result, logits, residual, counts,
                      attention_lora, moe_lora)
