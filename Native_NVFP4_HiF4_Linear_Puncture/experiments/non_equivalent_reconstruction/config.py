...
RESULTS = ROOT / "results/non_equivalent_reconstruction"
...
class Config:
    ...
    def validate(self) -> None:
        if self.matrix_sharing not in {"linear", "group"}:
            raise ValueError("matrix_sharing must be linear or group")
        if self.router_loss not in {"top_partial", "top_mass"}:
            raise ValueError("router_loss must be top_partial or top_mass")
        if self.calib_source != "s1k_original":
            raise ValueError("this experiment uses s1k_original")
        for name in ("epochs", "batch_size", "router_top_k", "calib_nsamples", "calib_val_nsamples"):
