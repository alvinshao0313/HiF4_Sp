...
RUN_ID=${RUN_ID:-$(date -u +%Y%m%dT%H%M%SZ)_r64_channel_diag_gradient}
...
conda run -n hif4 --no-capture-output \
  python -m Native_NVFP4_HiF4_Linear_Puncture.experiments.diag_gradient.run_r64_channel \
  --config "$CONFIG" \
