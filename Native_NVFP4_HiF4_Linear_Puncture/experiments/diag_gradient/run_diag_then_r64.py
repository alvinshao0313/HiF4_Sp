#!/usr/bin/env python3
"""先逐通道 DIAG、再 R64 块旋转（与 run_r64_channel 顺序相反）。

Y = Linear(Q_H((X*d) R), Q_H((W_N/d) R))
R64 = kron(kron(H4, H4), H4) / 8
"""
...
def forward_y_diag_then_r64(
    ...
    *,
    ...
    """先逐通道 DIAG，再 G64 右乘 R64，再两侧 HiF4。"""
    x_r = apply_r64_g64(x * d, compute_dtype=torch.float32, output_dtype=torch.float32)
    w_r = apply_r64_g64(w_n / d, compute_dtype=torch.float32, output_dtype=torch.float32)
    if use_ste:
...
def assert_unquant_diag_then_r64_equivalence(
    x: torch.Tensor,
    w_n: torch.Tensor,
    d: torch.Tensor,
    bias: torch.Tensor | None,
    ...
    x_s = x[:n]
    x_r = apply_r64_g64(x_s * d, compute_dtype=torch.float32, output_dtype=torch.float32)
    w_r = apply_r64_g64(w_n / d, compute_dtype=torch.float32, output_dtype=torch.float32)
    ...
    if rel > EQUIV_REL_L2_MAX:
        raise RuntimeError(
            f"未量化 DIAG→R64 乘积不等价: rel_L2={rel} > {EQUIV_REL_L2_MAX}"
        )
...
def optimize_one_module(
    *,
    ...
    with torch.no_grad():
        ...
        assert_unquant_diag_then_r64_equivalence(x_cal, w_n, d_one, bias)
        y_r_val = forward_y_diag_then_r64(
    ...
    for step in range(steps):
        ...
        y = forward_y_diag_then_r64(x_cal, w_n, d, bias, use_ste=True)
    ...
    with torch.no_grad():
        ...
        assert_unquant_diag_then_r64_equivalence(x_cal, w_n, d_final, bias)
        y_grad_val = forward_y_diag_then_r64(
...
def run(
    *,
    ...
    for module_name in module_names:
        print(f"[diag_then_r64] {module_name}", flush=True)
    ...
    csv_path = out_dir / "diag_then_r64_results.csv"
...
def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(description="先 DIAG 再 R64")
    ...
    run_id = args.run_id or (
        datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ") + "_diag_then_r64_gradient"
    )
    run(
