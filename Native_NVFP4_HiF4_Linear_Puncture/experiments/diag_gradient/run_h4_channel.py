#!/usr/bin/env python3
"""H4 块旋转 + 逐通道 DIAG 的梯度优化。

公式约定
--------
R4 = H4/2（正交），R = blockdiag(R4,...,R4)。
可学习 d 长度 K（旋转后坐标系，每通道独立）。
...
与 run_h4_group.py 的差别：这里不做四元组共享，d 长度是 K。
共享工具见 common.py。
"""
...
def assert_unquant_h4_channel_equivalence(
    x: torch.Tensor,
    w_n: torch.Tensor,
    d: torch.Tensor,
    bias: torch.Tensor | None,
    ...
    if rel > EQUIV_REL_L2_MAX:
        raise RuntimeError(
            f"未量化 H4+逐通道DIAG 乘积不等价: rel_L2={rel} > {EQUIV_REL_L2_MAX}"
        )
...
def optimize_one_module(
    *,
    ...
    with torch.no_grad():
        ...
        assert_unquant_h4_channel_equivalence(x_cal, w_n, d_one, bias)
        y_h4_val = forward_y_h4_channel(
    ...
    for step in range(steps):
        ...
        y = forward_y_h4_channel(x_cal, w_n, d, bias, use_ste=True)
    ...
    with torch.no_grad():
        ...
        assert_unquant_h4_channel_equivalence(x_cal, w_n, d_final, bias)
        y_grad_val = forward_y_h4_channel(
...
def run(
    *,
    ...
    for module_name in module_names:
        print(f"[h4_channel_diag] {module_name}", flush=True)
    ...
    csv_path = out_dir / "h4_channel_diag_results.csv"
...
def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(
        description="H4 + 逐通道 DIAG：HiF4 路径对齐 NV 参考输出"
    )
