"""Reasoning 数据集加载（默认本地缓存的 s1K-1.1_tokenized）与 chat collator。"""
...
logger = logging.getLogger("qad.data")

# 本机 HF cache 里已有 tokenized 版（含 question / deepseek_* 字段），无需再下原始 s1K-1.1
...
def row_to_messages(row: dict[str, Any], *, trace_source: str = "deepseek") -> list[dict[str, str]]:
    """把 S1K-1.1 行转成 Qwen chat messages。"""
    ...
    thinking = _as_text(row.get("deepseek_thinking_trajectory"), field="deepseek_thinking_trajectory")
...
def load_reasoning_dataset(
    *,
    ...
    """加载 HF datasets；本地路径优先于 hub name。默认走本地 cache（离线）。"""
...
class ChatSFTCollator:
    """chat messages → input_ids / labels / attention_mask；prompt 段 labels=-100。"""
    ...
    def __call__(self, features: Sequence[dict[str, Any]]) -> dict[str, torch.Tensor]:
        ...
        for feat in features:
            ...
            # Full conversation (return_dict=False -> List[int] on transformers>=5)
            ...
            if full_ids and not isinstance(full_ids[0], int):
                raise TypeError(f"apply_chat_template must return List[int], got {type(full_ids[0])}")

            # Prompt-only (user turn) for label masking
            ...
            if len(full_ids) > int(self.max_length):
                if not self.allow_truncate:
                    raise ValueError(
                        ...
                        "set allow_truncate=true to truncate (not recommended for full S1K trajectories)"
                    )
...
def build_reasoning_train_dataset(
    *,
    ...
    # Smoke log one sample conversion
    sample = dataset[0]
    logger.info(
