...
class LinearForward(Function):
    ...
    def backward(ctx, grad_out):
        ...
        grad_out_quant = quant_dequant_float(
            grad_out, qp_in, force_fp32=True) if quant_grad else grad_out  # [B, L, Cout]
        grad_out_quant_trans = quant_dequant_float(grad_out.flatten(
            0, -2).transpose(-1, -2).contiguous(), qp_in, force_fp32=True) if quant_grad else grad_out.flatten(0, -2).transpose(-1, -2)
