#!/usr/bin/env bash
# v2: real W4A4 full-MLP objective + sensitivity weights + true-HiF4 G4/refine.
# s1k calib (128 full samples). Then RTN + arc/mmlu + mmlu_pro.
set -euo pipefail
...
LOG="${EXP_DIR}/logs/perm_s1k_v2.log"
...
echo "[$(date --iso-8601=seconds)] === 1) v2 MLP reorder on s1k (real W4A4 objective) ===" | tee -a "${LOG}"
rm -rf "${REORDERED}"
