#!/usr/bin/env bash
# Full pipeline: MLP hierarchical perm → HiF4 W4A4 RTN → arc/mmlu (lm_eval) + mmlu_pro (lighteval).
# Baseline RTN numbers are taken from HiF4_exp/qwen35_4b_w4a4_proj_ablation results/full (not re-run).
set -euo pipefail

EXP_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "${EXP_DIR}/../../../.." && pwd)"
cd "${REPO_ROOT}"
# Block_Sparse first: eval_lm_eval_hif4a.py is behind a symlink and resolve() breaks its own path logic.
...
mkdir -p "${PERM_OUT}" "${OUT_DIR}" "$(dirname "${LOG}")" "${EXP_DIR}/tmp"
: > "${LOG}"

echo "[$(date --iso-8601=seconds)] === 1) MLP hierarchical reorder ===" | tee -a "${LOG}"
rm -rf "${REORDERED}"
...
echo "[$(date --iso-8601=seconds)] === 2) HiF4 W4A4 RTN on reordered model ===" | tee -a "${LOG}"
rm -rf "${CKPT_DIR}"
mkdir -p "${CKPT_DIR}"
